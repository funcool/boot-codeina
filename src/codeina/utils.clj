...
(defn ns-filter
  ...
  `codeina.reader/read-namespaces`), a sequence of namespaces to keep
...
(defn- find-file-in-repo
  "Given a classpath-relative file (as from the output of
   `codeina.reader/read-namespaces`), and a sequence of source directory paths,
...
(defn add-source-paths
  "Accepts a sequence of namespaces (generated by
   `codeina.reader/read-namespaces`), the project root, and a list of
